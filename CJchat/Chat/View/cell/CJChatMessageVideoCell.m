...
#import "CJChatMessageVideoCell.h"
...
@interface CJChatMessageVideoCell ()
...
@implementation CJChatMessageVideoCell
...
- (void)setModelFrame:(CJMessageFrame *)modelFrame
{
    ...
    NSString *path          = [[CJVideoManager shareManager] receiveVideoPathWithFileKey:[modelFrame.model.mediaPath.lastPathComponent stringByDeletingPathExtension]];
...
{
   __block NSString *path = [[CJVideoManager shareManager] videoPathForMP4:self.modelFrame.model.mediaPath];
    [self videoPlay:path];
}
...
-(void)reloadStart {
    ...
    NSString *path = [[CJVideoManager shareManager] videoPathForMP4:self.modelFrame.model.mediaPath];
...
-(void)stopVideo {
    _topBtn.hidden = NO;
    [[ZacharyPlayManager sharedInstance] cancelVideo:[[CJVideoManager shareManager] videoPathForMP4:self.modelFrame.model.mediaPath]];
}
