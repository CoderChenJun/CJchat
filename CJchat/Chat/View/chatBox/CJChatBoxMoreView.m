...
@interface CJChatBoxMoreView() <UIScrollViewDelegate>
...
@implementation CJChatBoxMoreView

- (id) initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
//        self.backgroundColor = CJColor(237, 237, 246);
        self.backgroundColor = CJColor(244, 244, 246);
        [self topLine];
        [self scrollView];
        [self pageControl];
    }
...
- (void)setItems:(NSMutableArray *)items
{
    ...
    int i = 0, page = 0;
    for (CJChatBoxMoreViewItem * item in _items) {
...
// 点击了某个Item
- (void) didSelectedItem:(CJChatBoxMoreViewItem *)sender
{
...
{
    [self.scrollView scrollRectToVisible:CGRectMake(pageControl.currentPage * UISCREEN_WIDTH, 0, UISCREEN_WIDTH, self.scrollView.height) animated:YES];
}
