...
@interface CJChatBox ()<UITextViewDelegate>
...
@implementation CJChatBox


- (id) initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        [self setBackgroundColor:CJColor(241, 241, 248)];
        ...
        self.status = CJChatBoxStatusNothing; // 起始状态
...
- (UIView *) topLine
{
    if (_topLine == nil) {
        ...
        [_topLine setBackgroundColor:CJColor(165, 165, 165)];
    }
    return _topLine;
}
...
{
    if (_moreButton == nil) {
        _moreButton = [[UIButton alloc] initWithFrame:CGRectMake(UISCREEN_WIDTH - CHATBOX_BUTTON_WIDTH, (HEIGHT_TABBAR - CHATBOX_BUTTON_WIDTH) / 2, CHATBOX_BUTTON_WIDTH, CHATBOX_BUTTON_WIDTH)];
...
{
    if (_talkButton == nil) {
        _talkButton = [[UIButton alloc] initWithFrame:CGRectMake(self.voiceButton.x + self.voiceButton.width + 4, self.height * 0.13, self.faceButton.x - self.voiceButton.x - self.voiceButton.width - 8, TEXTVIEW_HEIGHT)];
...
{
//    CJChatBoxStatus lastStatus = self.status;
    self.status = CJChatBoxStatusShowKeyboard; 
}
...
{
    CJChatBoxStatus lastStatus = self.status;
    if (lastStatus == CJChatBoxStatusShowVoice) {//正在显示talkButton，改为键盘状态
        self.status = CJChatBoxStatusShowKeyboard;
        ...
        self.status = CJChatBoxStatusShowVoice;
        [self.textView resignFirstResponder];
...
{
    CJChatBoxStatus lastStatus = self.status;
    if (lastStatus == CJChatBoxStatusShowMore) { // 当前显示的就是more页面
        self.status = CJChatBoxStatusShowKeyboard;
        [self.textView becomeFirstResponder];
    } else {
        [self.talkButton setHidden:YES];
        ...
        self.status = CJChatBoxStatusShowMore;
        if (lastStatus == CJChatBoxStatusShowFace) {  // 改变按钮样式
            [_faceButton setImage:[UIImage imageNamed:@"ToolViewEmotion"] forState:UIControlStateNormal];
        } else if (lastStatus == CJChatBoxStatusShowVoice) {
            [_talkButton setHidden:YES];
            [_textView setHidden:NO];
            [_voiceButton setImage:[UIImage imageNamed:@"ToolViewInputVoice"] forState:UIControlStateNormal];
        } else if (lastStatus == CJChatBoxStatusShowKeyboard) {
            [self.textView resignFirstResponder];
            self.status = CJChatBoxStatusShowMore;
        }
    }
...
{
    CJChatBoxStatus lastStatus = self.status;
    if (lastStatus == CJChatBoxStatusShowFace) {       // 正在显示表情,改为现实键盘状态
        self.status = CJChatBoxStatusShowKeyboard;
        [_faceButton setImage:[UIImage imageNamed:@"ToolViewEmotion"] forState:UIControlStateNormal];
        [self.textView becomeFirstResponder];
    } else {
...
{
    CJEmotion *emotion = notifi.userInfo[CJSelectEmotionKey];
    if (emotion.code) {
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(emotionDidSelected:) name:CJEmotionDidSelectNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deleteBtnClicked) name:CJEmotionDidDeleteNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(sendMessage) name:CJEmotionDidSendNotification object:nil];
}
