...
//  CJchat
//
//  Created by CoderChenJun on 16/3/21.
...
//    self.recordBtn.frame = CGRectMake(0, self.height-20-70, 70, 70);
    ...
    self.videoLayerView.frame = CGRectMake(0, 10, UISCREEN_WIDTH,self.recordBtn.top-20-10);
...
- (void)recordVideoStarted
{
    ...
    self.timeLine.frame = CGRectMake(0,self.videoLayerView.bottom , UISCREEN_WIDTH, 1);
    ...
    self.videoName = [NSString currentTimeString];
    [[CJVideoManager shareManager] startRecordingVideoWithFileName:self.videoName];
}
...
- (void)recordVideoFinished
{
    ...
    if ((double)timeInterval <= kDurationTime) { //小于或等于规定时间
        ...
        [[CJVideoManager shareManager] stopRecordingVideo:^(NSString *path) {
            NSDictionary *userInfo = @{VideoPathKey : path};
            [self routerEventWithName:CJRouterEventVideoRecordFinish userInfo:userInfo];
        }];
    } else {
        return;
    }
...
- (void)recordVideoCanceled
{
    ...
    [[CJVideoManager shareManager] stopRecordingVideo:^(NSString *path) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [[CJVideoManager shareManager] cancelRecordingVideoWithFileName:_videoName];
        });
    }];
}
...
- (void)exit
{
    ...
    [self routerEventWithName:CJRouterEventVideoRecordExit userInfo:nil];
}
...
{
    [self destroyTimer];
//    [[CJVideoManager shareManager] exit];// 预防直接返回的情况,但是目前还不行
//    CJLog(@"CJVideoView------dealloc");
}
