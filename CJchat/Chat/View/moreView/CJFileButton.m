...
- (void)setMessageModel:(CJMessageModel *)messageModel
{
    ...
    CJFileType type = [[dicLnk objectForKey:@"x"] intValue];
    NSString *name  = [dicLnk objectForKey:@"n"];
    self.imageV.image = [CJMessageHelper allocationImage:type];
...
- (UILabel *)nameLabel
{
    if (!_nameLabel) {
        ...
        _nameLabel.textColor = CJHexColor(0x262626);
...
- (UILabel *)sizeLabel
{
    if (!_sizeLabel) {
        ...
        _sizeLabel.textColor = CJHexColor(0x707070);
    }
    return _sizeLabel;
}
...
- (UILabel *)identLabel
{
    if (!_identLabel) {
        ...
        _identLabel.textColor = CJHexColor(0x707070);
