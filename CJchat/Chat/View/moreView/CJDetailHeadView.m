...
#import "CJDetailHeadView.h"
#import "CJDetailButton.h"


#define ICHeadMaxCols 4

@interface CJDetailHeadView ()
...
@implementation CJDetailHeadView
...
- (void)headBtnClicked:(CJDetailButton *)headBtn
{
...
- (void)setUsers:(NSArray *)users
{
    ...
    for (int i = 0; i < count; i ++) {
        CJUser *user            = _users[i];
        CJDetailButton *headBtn = [[CJDetailButton alloc] init];
...
//    self.topHeadBtn.centerX  = UISCREEN_WIDTH*0.5;
...
//    _nameLabel.centerX = _topHeadBtn.centerX;
    ...
    CGFloat btnH     = btnW;
    for (int i = 0; i < count; i ++) {
        CJDetailButton *btn = self.subviews[i];
        btn.width           = btnW;
        btn.height          = btnH;
...
- (UIView *)topView
{
    if (!_topView) {
        ...
        _topView.backgroundColor = CJHexColor(0x027996);
