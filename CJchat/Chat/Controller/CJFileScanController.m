...
#import "CJAudioView.h"

@interface CJFileScanController ()<QLPreviewControllerDataSource,QLPreviewControllerDelegate,UIWebViewDelegate,UIDocumentInteractionControllerDelegate>

@property (nonatomic, weak) UIWebView *webView;
@property (nonatomic, strong) QLPreviewController *previewController;
@property (nonatomic, strong) NSURL *fileURL;
@property (nonatomic, strong) UIDocumentInteractionController *documentInCtr;
@property (nonatomic, weak) CJAudioView *audioView;

@end

@implementation CJFileScanController
...
- (void)setupVew:(NSString *)type
{
    if ([type isEqualToString:@"html"] || [type isEqualToString:@"htm"]) {
        UIWebView *webView = [[UIWebView alloc] initWithFrame:CGRectMake(0, 0, self.view.width,UISCREEN_HEIGHT)];
        ...
        _previewController.view.frame = CGRectMake(0,0, self.view.width, UISCREEN_HEIGHT);
        ...
        CJAudioView *audioView = [[CJAudioView alloc] initWithFrame:self.view.bounds];
...
{
    UIImageView *imageV = [[UIImageView alloc] init];
    imageV.frame        = CGRectMake(UISCREEN_WIDTH*0.5-40, 45, 80, 80);
    imageV.image        = [UIImage imageNamed:@"iconfont-wenjian"];
    [self.view addSubview:imageV];
    UILabel *nameL      = [[UILabel alloc] initWithFrame:CGRectMake(UISCREEN_WIDTH*0.5-150, imageV.bottom+32, 300, 40)];
    nameL.text          = [self.filePath lastPathComponent];
    nameL.font          = [UIFont systemFontOfSize:15.0];
    nameL.textColor     = CJHexColor(0x505f62);
    nameL.numberOfLines = 0;
    nameL.textAlignment = NSTextAlignmentCenter;
    [self.view addSubview:nameL];
    [nameL sizeToFit];
    nameL.centerX = UISCREEN_WIDTH*0.5;
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(UISCREEN_WIDTH*0.5-100, nameL.bottom+85, 200, 40)];
    [self.view addSubview:label];
    label.numberOfLines = 0;
    label.font = [UIFont systemFontOfSize:14.0];
    label.textColor = CJHexColor(0xbebebe);
    label.textAlignment = NSTextAlignmentCenter;
    label.text = @"该文件暂不支持本地浏览，请使用其他应用打开";
    [label sizeToFit];
    label.centerX = UISCREEN_WIDTH * 0.5;
    UIButton *openBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    openBtn.frame     = CGRectMake(13, label.bottom + 40, UISCREEN_WIDTH-26, 48);
