...
#import "CJChatHearder.h"

@interface CJChatViewController ()<CJChatBoxViewControllerDelegate,UITableViewDelegate,UITableViewDataSource,CJRecordManagerDelegate,UIViewControllerTransitioningDelegate,UIViewControllerAnimatedTransitioning,BaseCellDelegate>
{
    CGRect _smallRect;
    CGRect _bigRect;
...
@property (nonatomic, strong) CJChatBoxViewController *chatBoxVC;
...
@property (nonatomic, strong) CJVoiceHud *voiceHud;


@end

@implementation CJChatViewController
...
- (void)setupUI
{
    self.view.backgroundColor = CJColor(240, 237, 237);
    // 注意添加顺序
    [self addChildViewController:self.chatBoxVC];
    [self.view addSubview:self.chatBoxVC.view];
    [self.view addSubview:self.tableView];
    
    self.tableView.backgroundColor = CJColor(240, 237, 237);
    // self.view的高度有时候是不准确的
    self.tableView.frame = CGRectMake(0, HEIGHT_NAVBAR+HEIGHT_STATUSBAR, self.view.width, UISCREEN_HEIGHT-HEIGHT_TABBAR-HEIGHT_NAVBAR-HEIGHT_STATUSBAR);
}

- (void)registerCell
{
    [self.tableView registerClass:[CJChatMessageTextCell class] forCellReuseIdentifier:TypeText];
    [self.tableView registerClass:[CJChatMessageImageCell class] forCellReuseIdentifier:TypePic];
    [self.tableView registerClass:[CJChatMessageVideoCell class] forCellReuseIdentifier:TypeVideo];
    [self.tableView registerClass:[CJChatMessageVoiceCell class] forCellReuseIdentifier:TypeVoice];
    [self.tableView registerClass:[CJChatMessageFileCell class] forCellReuseIdentifier:TypeFile];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([obj isKindOfClass:[NSString class]]) {
        ...
        CJMessageFrame *modelFrame     = (CJMessageFrame *)obj;
        NSString *ID                   = modelFrame.model.message.type;
        if ([ID isEqualToString:TypeSystem]) {
            CJChatSystemCell *cell = [CJChatSystemCell cellWithTableView:tableView reusableId:ID];
            cell.messageF              = modelFrame;
            return cell;
        }
        CJChatMessageBaseCell *cell    = [tableView dequeueReusableCellWithIdentifier:ID];
        cell.longPressDelegate         = self;
        [[CJMediaManager sharedManager] clearReuseImageMessage:cell.modelFrame.model];
...
{
    CJMessageFrame *messageF = [self.dataSource objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didEndDisplayingCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath*)indexPath {
    if ([cell isKindOfClass:[CJChatMessageVideoCell class]] && self) {
        CJChatMessageVideoCell *videoCell = (CJChatMessageVideoCell *)cell;
        [videoCell stopVideo];
    }
}

#pragma mark - CJChatBoxViewControllerDelegate
        ...
        didChangeChatBoxHeight:(CGFloat)height
{
    ...
    self.tableView.height = UISCREEN_HEIGHT - height - HEIGHT_NAVBAR - HEIGHT_STATUSBAR;
...
- (void)chatBoxViewController:(CJChatBoxViewController *)chatboxViewController didVideoViewAppeared:(CJVideoView *)videoView
{
    [_chatBoxVC.view setFrame:CGRectMake(0, UISCREEN_HEIGHT-HEIGHT_TABBAR, UISCREEN_WIDTH, UISCREEN_HEIGHT)];
    videoView.hidden = NO;
    [UIView animateWithDuration:0.5 animations:^{
        self.tableView.height = UISCREEN_HEIGHT - videwViewH - HEIGHT_NAVBAR-HEIGHT_STATUSBAR;
        self.chatBoxVC.view.frame = CGRectMake(0, videwViewX+HEIGHT_NAVBAR+HEIGHT_STATUSBAR, UISCREEN_WIDTH, videwViewH);
        [self scrollToBottom];
    } completion:^(BOOL finished) { // 状态改变
        self.chatBoxVC.chatBox.status = CJChatBoxStatusShowVideo;
        // 在这里创建视频设配
        UIView *videoLayerView = [videoView viewWithTag:1000];
        UIView *placeholderView = [videoView viewWithTag:1001];
        [[CJVideoManager shareManager] setVideoPreviewLayer:videoLayerView];
...
{
    CJMessageFrame *messageFrame = [CJMessageHelper createMessageFrame:TypeVideo senderName:@"马云" content:@"[视频]" path:videoPath from:@"gxz" to:self.group.gId fileKey:nil isSender:YES receivedSenderByYourself:NO]; // 创建本地消息
...
- (void)chatBoxViewController:(CJChatBoxViewController *)chatboxViewController sendFileMessage:(NSString *)fileName
{
    ...
    CJMessageFrame *messageFrame = [CJMessageHelper createMessageFrame:TypeFile senderName:@"马云" content:content path:fileName from:@"gxz" to:self.group.gId fileKey:nil isSender:YES receivedSenderByYourself:NO];
    NSString *path = [[CJFileTool fileMainPath] stringByAppendingPathComponent:fileName];
    double s = [CJFileTool fileSizeWithPath:path];
    NSNumber *x = [CJMessageHelper fileType:[fileName pathExtension]];
    if (!x) {
        x = @0;
    }
...
{
    CJMessageFrame *messageF = [CJMessageHelper createMessageFrame:TypeText senderName:@"马云" content:messageStr path:nil from:@"gxz" to:self.group.gId fileKey:nil isSender:YES receivedSenderByYourself:NO];
...
{
    CJMessageFrame *messageF = [CJMessageHelper createMessageFrame:TypeText senderName:@"马云" content:messageStr path:nil from:@"gxz" to:self.group.gId fileKey:nil isSender:NO receivedSenderByYourself:NO];
    [self addObject:messageF isSender:YES];
...
// 增加数据源并刷新
- (void)addObject:(CJMessageFrame *)messageF
...
- (void)messageSendSucced:(CJMessageFrame *)messageF
{
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        messageF.model.message.deliveryState = CJMessageDeliveryState_Delivered;
        [self.tableView reloadData];
    });
}
...
{
    CJMessageFrame *messageF = [CJMessageHelper createMessageFrame:TypePic senderName:@"马云" content:@"[图片]" path:imgPath from:@"gxz" to:self.group.gId fileKey:nil isSender:YES receivedSenderByYourself:NO];
    [self addObject:messageF isSender:YES];
...
- (void)chatBoxViewController:(CJChatBoxViewController *)chatboxViewController sendVoiceMessage:(NSString *)voicePath
{
    ...
    if (voicePath) {
        CJMessageFrame *messageF = [CJMessageHelper createMessageFrame:TypeVoice senderName:@"马云" content:@"[语音]" path:voicePath from:@"gxz" to:self.group.gId fileKey:nil isSender:YES receivedSenderByYourself:NO];
        [self addObject:messageF isSender:YES];
        [self messageSendSucced:messageF];
    }
}
...
- (void)longPress:(UILongPressGestureRecognizer *)longRecognizer
{
    if (longRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (![object isKindOfClass:[CJMessageFrame class]]) return;
        CJChatMessageBaseCell *cell = [self.tableView cellForRowAtIndexPath:indexPath];
...
{
    CJMessageFrame *modelFrame = [userInfo objectForKey:MessageKey];
    if ([eventName isEqualToString:CJRouterEventTextUrlTapEventName]) {
    } else if ([eventName isEqualToString:CJRouterEventImageTapEventName]) {
        ...
        NSString *orgImgPath = [[CJMediaManager sharedManager] originImgPath:modelFrame];
    ...
    } else if ([eventName isEqualToString:CJRouterEventVoiceTapEventName]) {
...
- (void)progressChange
{
    AVAudioRecorder *recorder = [[CJRecordManager shareManager] recorder] ;
...
// play voice
- (void)chatVoiceTaped:(CJMessageFrame *)messageFrame
             voiceIcon:(UIImageView *)voiceIcon
               redView:(UIView *)redView
{
    CJRecordManager *recordManager = [CJRecordManager shareManager];
    ...
    if (self.voicePath) {
        if ([self.voicePath isEqualToString:voicePath]) { // the same recoder
            self.voicePath = nil;
            [[CJRecordManager shareManager] stopPlayRecorder:voicePath];
    ...
    [[CJRecordManager shareManager] startPlayRecorder:voicePath];
...
#pragma mark - CJRecordManagerDelegate

- (void)voiceDidPlayFinished
{
    self.voicePath = nil;
    CJRecordManager *manager = [CJRecordManager shareManager];
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    if (self.presentFlag) {
        ...
        CJPhotoBrowserController *photoVC = (CJPhotoBrowserController *)[transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
...
- (void)showLargeImageWithPath:(NSString *)imgPath
                  withMessageF:(CJMessageFrame *)messageF
{
    UIImage *image = [[CJMediaManager sharedManager] imageWithLocalPath:imgPath];
    if (image == nil) {
        CJLog(@"image is not existed");
        return;
    }
    CJPhotoBrowserController *photoVC = [[CJPhotoBrowserController alloc] initWithImage:image];
...
- (void)showMenuViewController:(UIView *)showInView andIndexPath:(NSIndexPath *)indexPath message:(CJMessageModel *)messageModel
{
    ...
    NSInteger currentTime = [CJMessageHelper currentMessageTime];
    NSInteger interval    = currentTime - messageModel.message.date;
    if (messageModel.isSender) {
        if ((interval/1000) < 5*60 && !(messageModel.message.deliveryState == CJMessageDeliveryState_Failure)) {
...
- (void)copyMessage:(UIMenuItem *)copyMenuItem
{
    ...
    CJMessageFrame * messageF = [self.dataSource objectAtIndex:_longIndexPath.row];
...
- (void)recallMessage:(UIMenuItem *)recallMenuItem
{
    ...
    CJMessageFrame *msgF = [CJMessageHelper createMessageFrame:TypeSystem senderName:@"马云" content:@"你撤回了一条消息" path:nil from:@"gxz" to:self.group.gId fileKey:nil isSender:YES receivedSenderByYourself:NO];
    [self.dataSource insertObject:msgF atIndex:_longIndexPath.row];
    [self.tableView reloadData];
}


- (void)statusChanged:(CJMessageFrame *)messageF
{
    [self.dataSource removeObject:messageF];
...
{
    CJLog(@"需要用到的数据库，等添加了数据库再做转发...");
}

#pragma mark - Getter and Setter

- (CJChatBoxViewController *) chatBoxVC
{
    if (_chatBoxVC == nil) {
        _chatBoxVC = [[CJChatBoxViewController alloc] init];
        [_chatBoxVC.view setFrame:CGRectMake(0,UISCREEN_HEIGHT-HEIGHT_TABBAR, UISCREEN_WIDTH, UISCREEN_HEIGHT)];
...
{
    if (!_voiceHud) {
        _voiceHud = [[CJVoiceHud alloc] initWithFrame:CGRectMake(0, 0, 155, 155)];
        _voiceHud.hidden = YES;
        [self.view addSubview:_voiceHud];
        _voiceHud.center = CGPointMake(UISCREEN_WIDTH/2, UISCREEN_HEIGHT/2);
    }
    return _voiceHud;
}
