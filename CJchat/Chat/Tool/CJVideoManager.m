...
@interface CJVideoManager ()<AVCaptureFileOutputRecordingDelegate>
{
...
- (void)startRecordingVideoWithFileName:(NSString *)videoName
{
    ...
    if (!connection) {
        CJLog(@"capture connection wrong!");
        return;
    }
...
- (CGFloat)getFileSize:(NSString *)path
{
    ...
    CJLog(@"file size : %f",[outputFileAttributes fileSize]/1024.0/1024.0);
    ...
    NSURL *mp4Url = [self convertMp4:outputFileURL];
     ...
     [self compressVideo:[mp4Url path] finished:^(NSString *path) {
        if (path) {
         ...
         // 删除原录制的文件
         if ([CJFileTool fileExistsAtPath:outputFileURL.path]) {
             [CJFileTool removeFileAtPath:outputFileURL.path];
         }
    }];
    // 直接转大小不会改变
...
{
    NSString *videoPath   = [[CJVideoManager shareManager] videoPathWithFileName:[[namePath lastPathComponent] stringByDeletingPathExtension]];
