...
@implementation CJRecordManager
...
{
    NSError *error = nil;
    if (![[CJRecordManager shareManager] canRecord]) {
...
//    [self.recorder stop];
    ...
    if ([_recorder isRecording]) {
        if ([_endDate timeIntervalSinceDate:_startDate] < [self recordMinDuration]) {
            ...
            CJLog(@"record time duration is too short");
            return;
        }
        recordFinish = completion;
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [self.recorder stop];
            CJLog(@"record time duration :%f",[_endDate timeIntervalSinceDate:_startDate]);
        });
    }
}
...
{
    return [CJFileTool createPathWithChildPath:kChildPath];
}
...
- (void)audioRecorderEncodeErrorDidOccur:(AVAudioRecorder *)recorder
                                   error:(NSError *)error
{
    CJLog(@"audioRecorderEncodeErrorDidOccur");
}
